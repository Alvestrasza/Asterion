...
class Art:
    ...
    def paint(self,obj,palette,relief=.0015,palette_at=None):
        ...
        colors=[]
        def ridges(u,v):
            # Quiet, overlapping short-fur relief. Warping and per-row offsets
            # prevent a checkerboard, while geometry keeps a coherent flow.
            u+=.13*math.sin(v*1.73)+.09*math.sin(v*3.19+u*.64)
            v+=.12*math.sin(u*1.17)+.055*math.sin(u*3.70)
            row=math.floor(v);u+=.5*(row%2);column=math.floor(u)
            t=v-row;q=u-column-.5
            seed=math.sin(column*19.17+row*33.41)
            q+=.065*math.sin(math.pi*t)*seed
            width=(.41+.035*seed)*(.70+.30*math.sin(math.pi*t))
            ridge=max(0,1-(q/width)**2)**1.7*math.sin(math.pi*t)**.72
            ridge*=1-.13*math.exp(-(q/.040)**2)
            return ridge
        for vertex,p,n in zip(obj.data.vertices,positions,normals):
            ...
            fur=(wx*ridges(p.y*19,p.z*13+p.y*3)+
                 wy*ridges(p.x*23,p.z*16)+wz*ridges(p.x*21,p.y*14))/total
            vertex.co=inverse@(p+n*relief*fur)
            pal=palette_at(p) if palette_at else palette
            color=mix(pal[0],pal[2],.54+.10*f)
            color=mix(color,pal[3],.012*f)
    ...
    def lock(self,name,points,width,depth,palette,part='head',normal=(0,-1,0),steps=64,sides=44,root_palette=None):
        ...
        for i,c in enumerate(centers):
            ...
            for j in range(sides):
                ...
                sculpt=1+fade*(.045*math.cos(3*a)-.072*groove+.012*math.cos(19*a+t*2))
                verts.append(c+u*(width*radius*lateral*sculpt)+n*(depth*radius*outward*sculpt))
                light=max(0,outward);color=mix(palette[0],palette[2],.43+.37*light)
                stripe=math.exp(-((lateral-.22)/.4)**2)*light**2*fade
                color=mix(color,palette[3],.14*stripe);color=mix(color,palette[0],.10*groove*fade)
                if root_palette is not None:
                    color=mix(mix(root_palette[0],root_palette[2],.60),color,smooth(t/.64))
                colors.append(color[:3]+(1,))
    ...
    def eye(self,head,sign,palette,skin_palette,palette_at=None,width=.48,height=.55,z=2.49):
        ...
        # Seat the independent iris near the curved skull, rather than leaving
        # its tangent-plane rim in front of the orbital silhouette.
        c-=n*.048
        def coord(a,t=1):
        ...
        for row in range(13):
            ...
            for j in range(seg):
                ...
                # Share the exact outer iris boundary, not an overlapping
                # inset strip on a different plane (which caused edge flicker).
        ...
        for row in range(rings+1):
            ...
            for j in range(seg):
                ...
                ir=math.sqrt((x/(width*.447))**2+((zz+height*.012)/(height*.508))**2)
