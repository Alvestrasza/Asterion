"""Refine, reopen and independently import one immutable companion v002 master.
...
def main():
    ...
    master = REPO / 'assets/3d/source' / kind / 'sculpt-v002' / (kind + '-sculpt-v002.blend')
    model = REPO / 'public/assets/3d' / kind / (kind + '-sculpt-v002.glb')
    ...
    dependencies = [ROOT / name for name in ('build.py', 'refinements.py', 'export_equipment.py')] + [
        ...
        common.SOURCE_ROOT / 'asterion/sculpt-v004/export_equipment.py']
    ...
    for component, objects in groups.items():
        ...
        for obj in objects:
            ...
            for key, value in equipment.extras(kind, component).items():
                obj[key] = value
    if eye_signatures(g.ASSET) != eyes:
        raise ValueError('Original ocular geometry changed')
    print('EXPORT', kind, len(changes['edited_objects']), 'refined objects', flush=True)
    glb = out / (kind + '-sculpt-v003.glb'); blend = out / (kind + '-sculpt-v003.blend')
    ...
    bpy.context.scene['revision'] = 'sculpt-v003'
    ...
    checks['original_eye_geometry_colors_weights_unchanged'] = eyes == eye_signatures(meshes)
    ...
    checks['under_25_mb'] = glb.stat().st_size < 25000000
    ...
    bpy.context.scene.world = bpy.data.worlds.new('Companion v003 independent import studio')
    ...
    report = {'schema': 'asterion-local-modular-companion-v1', 'version': 'sculpt-v003', 'kind': kind,
        'name': refinements.NAMES[kind], 'source_sha256': verify.digest(blend), 'model_sha256': verify.digest(glb),
        'reference_sha256': verify.digest(reference), 'input_sha256': hashes, 'blender': bpy.app.version_string,
        **exported, 'geometry': geometry, 'triangle_count_evidence': count_evidence, 'changes': changes,
        ...
        'source_views': source_views, 'import_views': imported_views, 'checks': checks, 'passed': all(checks.values()),
        'human_likeness_accepted': False, 'mobile_performance_accepted': False, 'paid_provider_used': False,
        'projection_used': False, 'package_status': 'Local hash-checked Blender delivery; no game-dev canonical receipt.',
        ...
        'likeness_limit': 'Approved single-view reference retained. Hidden surfaces inferred; no exact 1:1 claim.'}
