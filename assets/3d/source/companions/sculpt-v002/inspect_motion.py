"""Independent companion v002 GLB motion diagnostics and six-view contact sheet.
...
COMMON_ROOT = ROOT.parent / 'sculpt-v001'
...
def contact_sheet(output, renders, kind):
    ...
    path = output / (kind+'-v002-motion-contact-sheet.png')
...
def main():
    ...
    for item in sample_reports:
        ...
        name = f"{kind}-{item['clip']}-f{item['frame']:03d}"
    ...
    report['contact_sheet'] = str(contact_sheet(output, report['renders'], kind))
    report_path.write_text(json.dumps(report, indent=2, allow_nan=False), encoding='utf-8')
