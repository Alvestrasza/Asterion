...
def main():
    ...
    if digest(master)!=master_sha256:raise ValueError('Master changed while being opened')
    ...
    meshes=[o for o in scene.objects if o.type=='MESH' and o not in helper_shapes]
    ...
    for obj in meshes:
        ...
        for vertex in obj.data.vertices:
            ...
            weights=[p for p in assignments if math.isfinite(p.weight) and p.weight>0]
            if not weights:unweighted+=1
            if abs(sum(p.weight for p in weights)-1)>.002 or any(not math.isfinite(p.weight) or p.weight<0 or p.weight>1.001 for p in assignments):invalid_weights+=1
    ...
    if digest(source)!=source_sha256:raise ValueError('GLB changed during validation')
    if digest(master)!=master_sha256:raise ValueError('Master changed during validation')
    report={
      'asset':str(source),'sha256':source_sha256,'master_sha256':master_sha256,
      ...
      'included_meshes':[o.name for o in meshes],
      'excluded_armature_visualization_helpers':[o.name for o in helper_shapes],
