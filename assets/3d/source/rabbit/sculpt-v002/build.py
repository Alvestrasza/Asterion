"""Build a new Liora v002 review; existing output GLBs are never overwritten."""
import argparse
import hashlib
import json
...
def main():
    ...
    glb=out/'rabbit-sculpt-v002.glb';blend=out/'rabbit-sculpt-v002.blend'
    if glb.exists() or blend.exists():raise FileExistsError('Use a fresh review directory')
    bpy.ops.wm.read_factory_settings(use_empty=True)
    bpy.context.scene.world=bpy.data.worlds.new('Liora neutral studio')
    g=common.toolkit()
    print('BUILD Liora anatomy',flush=True);spec=anatomy.build(g)
    # Stable pre-groom surfaces keep micro-fur normals out of metal fitting.
    g.ORNAMENT_SUPPORT={}
    for name in ('body','head'):
        source=anatomy.find(g,name);support=source.copy();support.data=source.data.copy()
        support.name='_LIORA_CLEAN_SUPPORT_'+name;bpy.context.collection.objects.link(support)
        support.hide_render=True;g.ORNAMENT_SUPPORT[name]=support
    print('BUILD Liora layered fur',flush=True);fur.build(g)
    print('BUILD Liora fitted eyes and muzzle',flush=True);anatomy.finish_face(g)
    print('BUILD Liora star tack',flush=True);ornaments.build(g)
    for support in g.ORNAMENT_SUPPORT.values():bpy.data.objects.remove(support,do_unlink=True)
    ...
    reference=REPO/'public'/'assets'/'companions'/'rabbit.png'
    digest=lambda p:hashlib.sha256(p.read_bytes()).hexdigest().upper()
    scene['reference_sha256']=digest(reference);scene['reference_authority']='Unchanged approved single-view rabbit.png'
    scene['companion_kind']='rabbit';scene['companion_name']='Liora';scene['revision']='sculpt-v002'
    scene['interpretation_limit']='Hidden views inferred; no exact 1:1 claim or mobile acceptance.'
    print('EXPORT Liora',flush=True)
    report=common.exporter().export_glb(glb,g.ASSET,rig)
    bpy.ops.wm.save_as_mainfile(filepath=str(blend))
    renders=common.render_views(g,cam,center,span,out,'rabbit',args.views.split(','))
